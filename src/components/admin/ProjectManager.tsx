...
export function ProjectManager() {
  ...
  const handleFormSubmit = async (data: any) => {
    ...
    try {
      ...
      const successMsg = `Project ${editingProject ? 'updated' : 'added'} successfully!`;
      ...
      const msg = 'Failed to save project.';
      setErrorMessage(msg);
      toast.error(msg);
    } finally {
      setIsLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const successMsg = deleteConfirm.type === 'single' ? 'Project deleted.' : `${selectedIds.size} projects deleted.`;
      setSuccessMessage(successMsg);
      toast.success(successMsg);
      ...
      const msg = 'Failed to delete project(s).';
      setErrorMessage(msg);
      toast.error(msg);
    } finally {
      setIsLoading(false);
    ...
    <div className="space-y-8 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-ink tracking-tight">Project Manager</h1>
            <p className="text-body font-medium mt-1">Showcase your best engineering work</p>
          </div>
        </div>
        <div className="flex gap-2 w-full md:w-auto">
          {selectedIds.size > 0 && (
            <button onClick={() => setDeleteConfirm({ type: 'bulk' })} className="flex-1 md:flex-none">
      ...
      <div className="grid grid-cols-1 gap-6">
        {filteredProjects.length === 0 ? (
          <div className="bg-surface-card border-2 border-dashed border-line p-16 text-center">
            <p className="text-mute font-medium">No projects found matching your criteria.</p>
          </div>
        ) : (
          filteredProjects.map((project) => (
            <div 
              ...
              className={`flex flex-col lg:flex-row items-stretch gap-6 bg-surface-card border border-line p-4 md:p-5 cursor-move
                ${selectedIds.has(project.id) ? 'bg-primary/5' : ''} 
                ${dragOverItem === project.id ? 'translate-y-2' : ''} 
                ${draggedItem === project.id ? 'opacity-50' : ''}`}
            >
              {/* Drag & Select Handles */}
              <div className="hidden md:flex flex-col items-center justify-center gap-4 px-2 border-r border-line/30">
                <GripVertical className="text-mute w-5 h-5" />
                <button 
                  onClick={(e) => { e.stopPropagation(); handleSelectOne(project.id); }}
                  className={`${selectedIds.has(project.id) ? 'text-primary' : 'text-stone hover:text-mute'}`}
                >
              ...
              {/* Project Image */}
              <div className="relative w-full lg:w-64 aspect-video lg:aspect-square overflow-hidden border border-line shrink-0 bg-surface-soft">
                {project.imageUrl ? (
                  <Image 
                ...
                <div className="absolute top-2 left-2">
                  <span className="px-3 py-1 bg-black/60 text-white text-[10px] font-black uppercase tracking-widest border border-white/10">
                    {project.category}
                  </span>
                </div>
              </div>
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-between py-2">
                <div>
                  <h3 className="text-2xl font-black text-ink truncate">
                    {project.title}
                  </h3>
                  <p className="text-sm text-body mt-2 line-clamp-2 md:line-clamp-3 leading-relaxed">
                    {project.description}
                  </p>
                  ...
                  {(project.tech || project.technologies) && (
                    <div className="flex flex-wrap gap-1.5 mt-4">
                      {(project.tech || project.technologies)?.slice(0, 5).map((t, idx) => (
                        <span key={idx} className="text-[10px] font-bold px-2 py-0.5 bg-surface-soft text-mute border border-line">
                          {t}
                        </span>
                      ))}
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-line/30">
                  <div className="flex items-center gap-3">
                    {project.links?.github && (
                      <a href={project.links.github} target="_blank" rel="noopener noreferrer" className="p-2 bg-surface-soft hover:text-primary">
                        <GithubIcon className="w-4 h-4" />
                      </a>
                    )}
                    {project.links?.live && (
                      <a href={project.links.live} target="_blank" rel="noopener noreferrer" className="p-2 bg-surface-soft hover:text-primary">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="h-9 px-4 hover:text-primary"
                    >
                      <Edit2 className="w-3.5 h-3.5 mr-2" /> Edit
                    </button>
                    <button
                      onClick={() => setDeleteConfirm({ type: 'single', item: project })}
                      className="h-9 px-4 hover:text-accent-red"
                    >
