...
const SectionHeader: React.FC<SectionHeaderProps> = ({ title, description, subtitle, center = false }) => {
  return (
    <div className={`mb-16 ${center ? 'text-center' : ''}`}>
      <motion.span 
        variants={fadeInUp}
        className="block text-primary font-semibold tracking-wider text-xl md:text-2xl mb-2 uppercase"
      >
        {subtitle}
      </motion.span>
      <motion.h2 
        variants={fadeInUp}
        className="text-lg md:text-xl font-bold text-ink dark:text-ink"
      >
        {title}
      </motion.h2>
      {description && (
