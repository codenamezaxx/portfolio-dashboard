---
version: "1.0"
name: "PostHog-Inspired Design System"
description: |
  A modern, professional design system inspired by PostHog with a custom gold accent color (#B8860B).
  The system uses a warm cream canvas (#eeefe9) with deep olive-charcoal text (#23251d) for headlines
  and olive-gray (#4d4f46) for body text. All components follow a flat design approach with no drop shadows,
  using thin hairline borders (#bfc1b7) instead. The gold accent (#B8860B) is used exclusively for primary
  CTAs, creating a cohesive and professional appearance across all pages.

## Color Palette

### Primary Colors
- **Primary CTA**: #B8860B (gold accent - user's original style)
- **Primary Pressed**: #9A6F08 (darker gold for hover/active states)
- **Primary Active**: #7A5506 (darkest gold for pressed state)
- **On Primary**: #23251d (deep olive-charcoal text on gold)

### Canvas & Surfaces
- **Canvas Background**: #eeefe9 (warm cream - page background)
- **Surface Card**: #ffffff (white - card backgrounds)
- **Surface Soft**: #e5e7e0 (soft cream - secondary buttons, input backgrounds)
- **Surface Doc**: #fcfcfa (warm white - doc article cards)
- **Surface Dark**: #23251d (deep olive-charcoal - code blocks)

### Text Colors
- **Ink (Headlines)**: #23251d (deep olive-charcoal)
- **Body**: #4d4f46 (olive-gray - default text)
- **Charcoal**: #33342d (emphasized text)
- **Mute**: #6c6e63 (metadata, secondary text)
- **Ash**: #9b9c92 (disabled text)
- **Stone**: #b6b7af (least-emphasis captions)

### Borders & Dividers
- **Hairline**: #bfc1b7 (1px card borders)
- **Hairline Soft**: #dcdfd2 (soft dividers)

### Semantic Colors
- **Link Blue**: #1d4ed8 (inline links)
- **Link Teal**: #1078a3 (doc links)
- **Accent Blue**: #2c84e0 (info callouts)
- **Accent Blue Soft**: #dceaf6 (info callout background)
- **Accent Red**: #cd4239 (warning callouts)
- **Accent Red Soft**: #f7d6d3 (warning callout background)
- **Accent Green**: #2c8c66 (success callouts)
- **Accent Green Soft**: #d9eddf (success callout background)
- **Accent Purple**: #7c44a6 (note callouts)
- **Accent Purple Soft**: #e7d8ee (note callout background)

## Typography

### Font Family
- **Primary**: IBM Plex Sans Variable (weights: 400, 500, 600, 700, 800)

### Typography Hierarchy
- **Display XL**: 36px, weight 700, line-height 1.5 (hero headlines)
- **Display LG**: 24px, weight 800, line-height 1.33, letter-spacing -0.6px (section headlines)
- **Heading LG**: 21px, weight 700, line-height 1.4, letter-spacing -0.5px (sub-section headings)
- **Heading MD**: 20px, weight 700, line-height 1.4 (card titles)
- **Heading SM**: 18px, weight 700, line-height 1.5 (section eyebrows - uppercase)
- **Body MD**: 16px, weight 400, line-height 1.5 (default body text)
- **Body Strong**: 16px, weight 600, line-height 1.5 (emphasis)
- **Body SM**: 15px, weight 400, line-height 1.71 (secondary text)
- **Body XS**: 14px, weight 500, line-height 1.43 (metadata)
- **Button MD**: 14px, weight 700, line-height 1.5 (button labels)
- **Code**: 14px, weight 400, line-height 1.43 (code blocks)
...
- **XXL**: 32px (double spacing)
- **Section**: 80px (section rhythm)

## Border Radius

- **None**: 0px (structural surfaces)
- **XS**: 2px (inline code chips)
- **SM**: 4px (form inputs, micro chips)
- **MD**: 6px (cards, buttons, standard components)
- **LG**: 8px (large containers)
- **Full**: 9999px (pill buttons, fully rounded)

## Component Specifications

### Buttons
- **Primary**: Gold (#B8860B) background, deep olive text, 40px height, 6px radius
- **Secondary**: Soft cream (#e5e7e0) background, ink text, 40px height, 6px radius
- **Tertiary**: Transparent background, ink text, 6px radius
- **Disabled**: Soft cream background, ash text, 6px radius

### Cards
- **Product Card**: White background, 1px hairline border, 24px padding, 6px radius
- **Doc Card**: Warm white background, 1px hairline border, 24px padding, 6px radius
- **Feature Tile**: White background, 1px hairline border, 20px padding, 6px radius
- **Pricing Card**: White background, 1px hairline border, 32px padding, 6px radius

### Inputs
- **Text Input**: White background, 1px hairline border, 36px height, 6px radius
- **Focused**: 2px accent-blue border, focus ring
- **Search Input**: White background, 1px hairline border, 36px height, 6px radius

### Navigation
- **Primary Nav**: Cream canvas background, 56px height, no radius
- **Sub Nav**: Soft cream background, 40px height, no radius
- **Doc Sidebar**: Cream canvas background, 240px width, no radius

### Badges & Pills
- **Badge Uppercase**: Transparent background, body text, uppercase
- **Badge Promo**: Accent blue soft background, link blue text, fully rounded
- **Pill Tab**: Transparent background, body text, fully rounded
- **Pill Tab Active**: Ink background, white text, fully rounded

## Design Principles

1. **No Drop Shadows**: Cards sit flat on cream with thin borders only
2. **Minimal Color**: Gold is the only saturated CTA color
3. **Cream Canvas**: Continuous cream background throughout
4. **Flat Design**: No gradients, no atmospheric effects
5. **Typography Hierarchy**: Weight-based hierarchy, not size-based
6. **Consistent Spacing**: 8px base unit throughout
7. **Accessible**: WCAG AA compliance, proper contrast ratios

## Implementation

All colors are defined in `src/app/globals.css` using CSS custom properties and Tailwind CSS 4 theme configuration.
Components use these colors through Tailwind utility classes (e.g., `bg-primary`, `text-foreground`, `border-hairline`).

## Accessibility

- All text meets WCAG AA contrast ratio requirements
- Focus states are clearly visible with 2px accent-blue borders
- Disabled states use reduced opacity and ash text color
